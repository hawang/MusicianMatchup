...
-----------------
     ...
     chomp!, chop, chop, chop!, chop!, concat, count, crypt, delete,
     delete, delete!, delete!, downcase, downcase!, dump, each,
     each_byte, each_char, each_line, empty?, end_regexp, eql?,
     expand_ch_hash, gsub, gsub!, hash, hex, include?, index,
     initialize_copy, insert, inspect, intern, is_binary_data?,
     is_complex_yaml?, iseuc, issjis, isutf8, jcount, jlength, jsize,
     kconv, length, ljust, lstrip, lstrip!, match, mbchar?, next, next!,
     oct, original_succ, original_succ!, quote, replace, reverse,
     reverse!, rindex, rjust, rstrip, rstrip!, scan, scanf, size, slice,
     slice!, split, squeeze, squeeze, squeeze!, squeeze!, strip, strip!,
     sub, sub!, succ, succ, succ!, succ!, sum, swapcase, swapcase!,
     to_f, to_i, to_s, to_str, to_sym, to_yaml, toeuc, tojis, tosjis,
     toutf16, toutf8, tr, tr, tr!, tr!, tr_s, tr_s, tr_s!, tr_s!,
